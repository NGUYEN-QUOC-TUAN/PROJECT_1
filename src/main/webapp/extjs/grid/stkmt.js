...
Ext.onReady(function() {
	...
	var orgdept, orgdate, orgnbr, orgvend;
	var row, newrecord;
	var params;
		...
		items: [{
			...
			handler: function() {
				...
				var rec = new Stock({
					...
					po: '',
					expiry_date: '',
					label_printed: cbfld4.getAt(1).data.val,
	...
	function clearFormAdd() {
		Ext.getCmp('um').setValue('');
		Ext.getCmp('currs').setValue('');
		Ext.getCmp('cost').setValue('');
		Ext.getCmp('vendors').setValue('');
	}
	function getComboVendor() {
		var params = {
			'nbr': Ext.getCmp('items').getValue(),
			'sub': 'bindVendor'
		};
		...
		Ext.Ajax.request({
			...
			success: function(response) {
				...
				vendorStore.loadData(jsonData.vendors);
			}
		});
	}
	...
	var itemStoreDisplay = new Ext.data.Store({
		model: 'itemModel',
		proxy: {
			type: 'ajax',
			url: 'stock?sub=bindSortByItem',
			reader: {
				type: 'json',
				root: 'itemsorts'
			}
	...
	var grid = Ext.create('Ext.grid.Panel', {
		...
		columns: [
					...
					listeners: {
						select: function() {
							if (!(Ext.getCmp('depts').getValue() == 'STORE'
								...
								var vendorValue = Ext.getCmp('vendors').getValue();
								var itemsValue = Ext.getCmp('items').getValue();

								var targetIndex = itemStore.findBy(function(record) {
									return record.get('vendor') === vendorValue && record.get('val') === itemsValue;
								});

								if (targetIndex !== -1) {
									Ext.getCmp('um').setValue(itemStore.getAt(targetIndex).get('um'));
									Ext.getCmp('currs').setValue(itemStore.getAt(targetIndex).get('curr'));
									Ext.getCmp('cost').setValue(itemStore.getAt(targetIndex).get('cost'));
								}
			...
			{
				header: 'UM', dataIndex: 'um', maxLength: 10,
				field: {
					id: "um",
					xtype: 'textfield',
					typeAhead: false,
