$(document).ready(function(){
	$("<h1/>").text("Department Auxiliary Materials Cost Bill").css('text-align',"center").appendTo($("body"));
	...
	$("<button/>").attr({'type': 'submit'}).text("comfirm").appendTo(headDiv);
	...
	hiddenForm.attr("action",ctx+'/deptCostBill!output');
	...
	function getList(){
		...
		$.getJSON( ctx + "/deptCostBill!getList",params, function(data){
			$('table').remove();
			getTable(data.rows);
		});
	...
	var rowGroupArray = ["tx_curr","dept_desc"];
	var colGroupArray = ["cat_col"];
	var colArray;
	...
	function getTotal(totalObject){
		$.each(totalObject , function(key , val){
			...
			for(var index = 0 ;index < val.index ; index ++){
				sumTotalTr.append($("<td/>").html("&nbsp;").css("border-top",0).css("border-bottom",0));
			}
