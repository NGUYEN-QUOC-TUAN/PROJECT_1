 ...
 * @version 2015年5月8日 下午4:29:14
 **/
@Repository("printMaterialDao")
public class PrintMaterialDao extends BaseDao{
	...
	public List<SqlRowValue> getList(String reportStartDate,
		...
		try {
			return getResult(" SELECT `cat_mstr`.`cat_code`, `tx_det`.`tx_qty`, `tx_det`.`tx_cost`, `tx_det`.`tx_date`, `tx_det`.`tx_dept`, `cat_mstr`.`cat_desc`, `item_mstr`.`item_desc`, `dept_mstr`.`dept_desc`, `tx_det`.`tx_curr`, `tx_det`.`tx_group`"
							...
							+ " WHERE  `cat_mstr`.`cat_code`='08' AND `tx_det`.`tx_dept`<>'STOCK' AND `tx_det`.`tx_group`='ADMIN' AND (`tx_det`.`tx_date`>={d '" + reportStartDate + "'} AND `tx_det`.`tx_date`<={d '" + reportEndDate + "'}) ORDER BY tx_curr ASC , item_desc ASC , dept_desc ASC");
