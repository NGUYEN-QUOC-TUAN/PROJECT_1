$(document).ready(function(){
	$("<h1/>").text("Stationery Report").css('text-align',"center").appendTo($("body"));
	...
	hiddenForm.attr("action",ctx+'/applyStationery!output');
	...
	function getList(){
		...
		$.getJSON( ctx + "/applyStationery!getList",params, function(data){
			$('table').remove();
			getTable(data.rows);
		});
		queryStatus = true;
